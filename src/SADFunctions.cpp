...
static unsigned int Satd_C(const uint8_t *pSrc, intptr_t nSrcPitch, const uint8_t *pRef, intptr_t nRefPitch) {
    if (nBlkWidth == 4 && nBlkHeight == 4)
        ...
        // 64-bit accumulator: for 16-bit pixels a large block (e.g. 128x128) can sum past
        // UINT32_MAX. Saturate on return since the SADFunction signature returns unsigned int
        // (the result is stored in an int64_t cost upstream); wrapping would make a maximally
        // different block look cheap.
        uint64_t sum = 0;
