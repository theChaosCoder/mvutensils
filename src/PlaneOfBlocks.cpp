...
static const uint8_t *pobGetRefBlock(PlaneOfBlocks *pob, int nVx, int nVy) {
    if constexpr (nLogPel == 0)
        return pob->pRefFrame->planes[0].GetAbsolutePointerPel1<PixelType>(pob->x[0] + nVx, pob->y[0] + nVy);
    else if constexpr (nLogPel == 1)
        return pob->pRefFrame->planes[0].GetAbsolutePointerPel2<PixelType>(pob->x[0] * 2 + nVx, pob->y[0] * 2 + nVy);
    else if constexpr (nLogPel == 2)
        return pob->pRefFrame->planes[0].GetAbsolutePointerPel4<PixelType>(pob->x[0] * 4 + nVx, pob->y[0] * 4 + nVy);
    return nullptr;
}
...
static const uint8_t *pobGetRefBlockU(PlaneOfBlocks *pob, int nVx, int nVy) {
    ...
    if constexpr (nLogPel == 0)
        return pob->pRefFrame->planes[1].GetAbsolutePointerPel1<PixelType>(
                pob->x[1] + ((nVx + xbias) >> pob->nLogxRatioUV),
                pob->y[1] + ((nVy + ybias) >> pob->nLogyRatioUV));
    else if constexpr (nLogPel == 1)
        return pob->pRefFrame->planes[1].GetAbsolutePointerPel2<PixelType>(
                pob->x[1] * 2 + ((nVx + xbias) >> pob->nLogxRatioUV),
                pob->y[1] * 2 + ((nVy + ybias) >> pob->nLogyRatioUV));
    else if constexpr (nLogPel == 2)
        return pob->pRefFrame->planes[1].GetAbsolutePointerPel4<PixelType>(
...
static const uint8_t *pobGetRefBlockV(PlaneOfBlocks *pob, int nVx, int nVy) {
    ...
    if constexpr (nLogPel == 0)
        return pob->pRefFrame->planes[2].GetAbsolutePointerPel1<PixelType>(
                pob->x[2] + ((nVx + xbias) >> pob->nLogxRatioUV),
                pob->y[2] + ((nVy + ybias) >> pob->nLogyRatioUV));
    else if constexpr (nLogPel == 1)
        return pob->pRefFrame->planes[2].GetAbsolutePointerPel2<PixelType>(
                pob->x[2] * 2 + ((nVx + xbias) >> pob->nLogxRatioUV),
                pob->y[2] * 2 + ((nVy + ybias) >> pob->nLogyRatioUV));
    else if constexpr (nLogPel == 2)
        return pob->pRefFrame->planes[2].GetAbsolutePointerPel4<PixelType>(
...
static inline FORCE_INLINE void pobCheckMV_Template(PlaneOfBlocks *pob, int vx, int vy, int *dir, int val) {
    if (pobIsVectorOK(pob, vx, vy)) {
        ...
        const uint8_t *blocks[] = {
            pobGetRefBlock<nLogPel, PixelType>(pob, vx, vy),
            pob->chroma ? pobGetRefBlockU<nLogPel, PixelType>(pob, vx, vy) : nullptr,
            pob->chroma ? pobGetRefBlockV<nLogPel, PixelType>(pob, vx, vy) : nullptr,
        };
...
static inline FORCE_INLINE void pobCheckMV0(PlaneOfBlocks *pob, int vx, int vy) { //here the chance for default values are high especially for zeroMVfieldShifted (on left/top border)
    pobCheckMV_Template<useSatd, nLogPel, CHECKMV_UPDATEBESTMV, PixelType>(pob, vx, vy, 0, 0);
}
...
static inline FORCE_INLINE void pobCheckMV(PlaneOfBlocks *pob, int vx, int vy) { //here the chance for default values are high especially for zeroMVfieldShifted (on left/top border)
    pobCheckMV_Template<useSatd, nLogPel, CHECKMV_PENALTYNEW | CHECKMV_UPDATEBESTMV, PixelType>(pob, vx, vy, 0, 0);
}
...
static inline FORCE_INLINE void pobCheckMV2(PlaneOfBlocks *pob, int vx, int vy, int *dir, int val) {
    pobCheckMV_Template<useSatd, nLogPel, CHECKMV_PENALTYNEW | CHECKMV_UPDATEDIR | CHECKMV_UPDATEBESTMV, PixelType>(pob, vx, vy, dir, val);
}
...
static inline FORCE_INLINE void pobCheckMVdir(PlaneOfBlocks *pob, int vx, int vy, int *dir, int val) {
    pobCheckMV_Template<useSatd, nLogPel, CHECKMV_PENALTYNEW | CHECKMV_UPDATEDIR, PixelType>(pob, vx, vy, dir, val);
}
...
void pobInit(PlaneOfBlocks *pob, int _nBlkX, int _nBlkY, int _nBlkSizeX, int _nBlkSizeY, int _nPel, int _nLevel, bool smallestPlane, bool chroma, int _nOverlapX, int _nOverlapY, int _xRatioUV, int _yRatioUV, int bitsPerSample) {
    ...
    pob->smallestPlane = smallestPlane;
    pob->chroma = chroma;
...
static void pobDiamondSearch(PlaneOfBlocks *pob, int length) {
    ...
    while (direction > 0) {
        ...
        if (lastDirection & Right)
            pobCheckMV2<useSatd, nLogPel, PixelType>(pob, dx + length, dy, &direction, Right);
        if (lastDirection & Left)
            pobCheckMV2<useSatd, nLogPel, PixelType>(pob, dx - length, dy, &direction, Left);
        if (lastDirection & Down)
            pobCheckMV2<useSatd, nLogPel, PixelType>(pob, dx, dy + length, &direction, Down);
        if (lastDirection & Up)
            pobCheckMV2<useSatd, nLogPel, PixelType>(pob, dx, dy - length, &direction, Up);
        ...
        // diagonals to be checked, because we might be lucky.
            ...
            switch (lastDirection) {
                case Right:
                    pobCheckMV2<useSatd, nLogPel, PixelType>(pob, dx + length, dy + length, &direction, Right + Down);
                    pobCheckMV2<useSatd, nLogPel, PixelType>(pob, dx + length, dy - length, &direction, Right + Up);
                    break;
                case Left:
                    pobCheckMV2<useSatd, nLogPel, PixelType>(pob, dx - length, dy + length, &direction, Left + Down);
                    pobCheckMV2<useSatd, nLogPel, PixelType>(pob, dx - length, dy - length, &direction, Left + Up);
                    break;
                case Down:
                    pobCheckMV2<useSatd, nLogPel, PixelType>(pob, dx + length, dy + length, &direction, Right + Down);
                    pobCheckMV2<useSatd, nLogPel, PixelType>(pob, dx - length, dy + length, &direction, Left + Down);
...
static void pobExpandingSearch(PlaneOfBlocks *pob, int r, int s, int mvx, int mvy) {
    ...
    {
        pobCheckMV<useSatd, nLogPel, PixelType>(pob, mvx + i, mvy - r);
        pobCheckMV<useSatd, nLogPel, PixelType>(pob, mvx + i, mvy + r);
    }

    for (j = -r + s; j < r; j += s) {
        pobCheckMV<useSatd, nLogPel, PixelType>(pob, mvx - r, mvy + j);
        pobCheckMV<useSatd, nLogPel, PixelType>(pob, mvx + r, mvy + j);
    }

    // then corners - they are more far from cenrer
    pobCheckMV<useSatd, nLogPel, PixelType>(pob, mvx - r, mvy - r);
    pobCheckMV<useSatd, nLogPel, PixelType>(pob, mvx - r, mvy + r);
    pobCheckMV<useSatd, nLogPel, PixelType>(pob, mvx + r, mvy - r);
    pobCheckMV<useSatd, nLogPel, PixelType>(pob, mvx + r, mvy + r);
}
...
static void pobHex2Search(PlaneOfBlocks *pob, int i_me_range) { //adopted from x264
    ...
    if (i_me_range > 1) {
        ...
        pobCheckMVdir<useSatd, nLogPel, PixelType>(pob, bmx - 2, bmy, &dir, 0);
        pobCheckMVdir<useSatd, nLogPel, PixelType>(pob, bmx - 1, bmy + 2, &dir, 1);
        pobCheckMVdir<useSatd, nLogPel, PixelType>(pob, bmx + 1, bmy + 2, &dir, 2);
        pobCheckMVdir<useSatd, nLogPel, PixelType>(pob, bmx + 2, bmy, &dir, 3);
        pobCheckMVdir<useSatd, nLogPel, PixelType>(pob, bmx + 1, bmy - 2, &dir, 4);
        pobCheckMVdir<useSatd, nLogPel, PixelType>(pob, bmx - 1, bmy - 2, &dir, 5);
        ...
        if (dir != -2) {
            ...
            for (int i = 1; i < i_me_range / 2 && pobIsVectorOK(pob, bmx, bmy); i++) {
                ...
                pobCheckMVdir<useSatd, nLogPel, PixelType>(pob, bmx + hex2[odir + 0][0], bmy + hex2[odir + 0][1], &dir, odir - 1);
                pobCheckMVdir<useSatd, nLogPel, PixelType>(pob, bmx + hex2[odir + 1][0], bmy + hex2[odir + 1][1], &dir, odir);
                pobCheckMVdir<useSatd, nLogPel, PixelType>(pob, bmx + hex2[odir + 2][0], bmy + hex2[odir + 2][1], &dir, odir + 1);
                if (dir == -2)
                    break;
                bmx += hex2[dir + 1][0];
                bmy += hex2[dir + 1][1];
    ...
    pobExpandingSearch<useSatd, nLogPel, PixelType>(pob, 1, 1, bmx, bmy);
}
...
static void pobCrossSearch(PlaneOfBlocks *pob, int start, int x_max, int y_max, int mvx, int mvy) { // part of umh  search
    ...
    for (int j = start; j < y_max; j += 2) {
        pobCheckMV<useSatd, nLogPel, PixelType>(pob, mvx, mvy - j);
        pobCheckMV<useSatd, nLogPel, PixelType>(pob, mvx, mvy + j);
    }
}
...
static void pobUMHSearch(PlaneOfBlocks *pob, int i_me_range, int omx, int omy) { // radius
    ...
    pobCrossSearch<useSatd, nLogPel, PixelType>(pob, 1, i_me_range, i_me_range, omx, omy);
    ...
    do {
        ...
        for (int j = 0; j < 16; j++) {
            ...
            pobCheckMV<useSatd, nLogPel, PixelType>(pob, mx, my);
        }
    } while (++i <= i_me_range / 4);

    //            if( bmy <= mv_y_max )
    //                goto me_hex2;
    pobHex2Search<useSatd, nLogPel, PixelType>(pob, i_me_range);
}
...
static void pobRefine(PlaneOfBlocks *pob) {
    ...
    if (pob->searchType == SearchType::Vertical) {
        ...
        int mvy = pob->bestMV.y;
        for (int i = 1; i <= pob->nSearchParam; i++) {
            pobCheckMV<useSatd, nLogPel, PixelType>(pob, mvx, mvy - i);
            pobCheckMV<useSatd, nLogPel, PixelType>(pob, mvx, mvy + i);
        }
    }
}
...
static void pobPseudoEPZSearch(PlaneOfBlocks *pob, int blkIdx, int blkx, int blky, int blkScanDir, bool tryMany, int &badcount) {
    ...
    const uint8_t *zeroMVBlocks[3] = {
        pobGetRefBlock<nLogPel, PixelType>(pob, 0, pob->zeroMVfieldShifted.y),
        pob->chroma ? pobGetRefBlockU<nLogPel, PixelType>(pob, 0, 0) : nullptr,
    ...
    const uint8_t *globalPredBlocks[3] = {
        pobGetRefBlock<nLogPel, PixelType>(pob, pob->globalMVPredictor.x, pob->globalMVPredictor.y),
        pob->chroma ? pobGetRefBlockU<nLogPel, PixelType>(pob, pob->globalMVPredictor.x, pob->globalMVPredictor.y) : nullptr,
        pob->chroma ? pobGetRefBlockV<nLogPel, PixelType>(pob, pob->globalMVPredictor.x, pob->globalMVPredictor.y) : nullptr,
    };
...
#define BADCOUNT_LIMIT 16
    ...
    if (blkIdx > 1 && foundSAD > (pob->badSAD + pob->badSAD * badcount / BADCOUNT_LIMIT)) {
        ...
        if (pob->badrange > 0) { // UMH
            ...
            pobUMHSearch<useSatd, nLogPel, PixelType>(pob, pob->badrange * (1 << nLogPel), 0, 0);
        } else if (pob->badrange < 0) { // ESA
            for (int i = 1; i < -pob->badrange * (1 << nLogPel); i += (1 << nLogPel)) { // at radius
                pobExpandingSearch<useSatd, nLogPel, PixelType>(pob, i, 1 << nLogPel, 0, 0);
    ...
    for (int blky = 0; blky < pob->nBlkY; blky++) {
        ...
        for (int iblkx = 0; iblkx < pob->nBlkX; iblkx++) {
            ...
            if (iblkx < pob->nBlkX - 1) {
                pob->x[0] += (pob->nBlkSizeX - pob->nOverlapX) * blkScanDir;
                if (chroma) {
                    pob->x[1] += ((pob->nBlkSizeX - pob->nOverlapX) >> pob->nLogxRatioUV) * blkScanDir;
                    pob->x[2] += ((pob->nBlkSizeX - pob->nOverlapX) >> pob->nLogxRatioUV) * blkScanDir;
                }
            }
        }
        pBlkData += pob->nBlkX;
...
static constexpr decltype(&doPobSearchMVs<false, 0, uint8_t>) doPobSearchMVs_Table8[] = {
    doPobSearchMVs<false, 0, uint8_t>, doPobSearchMVs<false, 1, uint8_t>, doPobSearchMVs<false, 2, uint8_t>,
    doPobSearchMVs<true, 0, uint8_t>, doPobSearchMVs<true, 1, uint8_t>, doPobSearchMVs<true, 2, uint8_t>
};

static constexpr decltype(&doPobSearchMVs<false, 0, uint8_t>) doPobSearchMVs_Table16[] = {
    doPobSearchMVs<false, 0, uint16_t>, doPobSearchMVs<false, 1, uint16_t>, doPobSearchMVs<false, 2, uint16_t>,
    doPobSearchMVs<true, 0, uint16_t>, doPobSearchMVs<true, 1, uint16_t>, doPobSearchMVs<true, 2, uint16_t>
};
...
static const decltype(&doPobRecalculateMVs<false, 0, uint8_t>) doPobRecalculateMVs_Table8[] = {
    doPobRecalculateMVs<false, 0, uint8_t>, doPobRecalculateMVs<false, 1, uint8_t>, doPobRecalculateMVs<false, 2, uint8_t>,
    doPobRecalculateMVs<true, 0, uint8_t>, doPobRecalculateMVs<true, 1, uint8_t>, doPobRecalculateMVs<true, 2, uint8_t>,
};

static const decltype(&doPobRecalculateMVs<false, 0, uint8_t>) doPobRecalculateMVs_Table16[] = {
    doPobRecalculateMVs<false, 0, uint16_t>, doPobRecalculateMVs<false, 1, uint16_t>, doPobRecalculateMVs<false, 2, uint16_t>,
    doPobRecalculateMVs<true, 0, uint16_t>, doPobRecalculateMVs<true, 1, uint16_t>, doPobRecalculateMVs<true, 2, uint16_t>,
};
...
void pobRecalculateMVs(PlaneOfBlocks *pob, const FakeGroupOfPlanes *fgop, const FramePyramidLevel *pSrcFrame, const FramePyramidLevel *pRefFrame,
    ...
    if (pob->bytesPerSample == 1)
        ...
        doPobRecalculateMVs_Table16[index](pob, fgop, pSrcFrame, pRefFrame, st, stp, lambda, pnew, out, fieldShift, thSAD, smooth, meander);
}
