...
void MotionBlockLevel::FetchPredictors(int blkidx, int blkx, int blky, int blkScanDir, VECTOR predictors[5]) noexcept {
    ...
    // Clamp the denominator to >= 1: with lsad == 0 (LSAD == 0) and a zero/sentinel
    // predictor SAD this would otherwise be 0/0 = NaN (or x/0 = inf), and the
    // assignment to the integer nLambda would be undefined behaviour.
...
void MotionBlockLevel::InitMotionEstimationFields(bool useSatd, bool chroma, int bytesPerSample) {
    ...
    if (useSatd && (nBlkSizeX == 16 && nBlkSizeY == 2))
        throw MotionBlockPyramidError("Unsupported block size for SATD");
...
void MotionBlockPyramid::ScaleThSCD(int64_t &thscd1, int &thscd2, int bitsPerSample) const {
    ...
    thscd2 = (int)std::min<int64_t>((int64_t)thscd2 * nBlkX * nBlkY / 256, INT32_MAX);
}
